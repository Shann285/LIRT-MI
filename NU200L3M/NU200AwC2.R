...
setwd("C:/Users/dell/Desktop/NU200L3M")
...
1,1.25,0.95,0.85,1.0,1.15,1.05,0.75,
1,1.25-0.4,0.95-0.4,0.85-0.4,1.0,1.15,1.05,0.75,
1,1.25-0.4,0.95-0.4,0.85-0.4,1.0,1.15,1.05,0.75
),nrow=T,ncol=I,byr=T)
...
0,0.25,-0.15,-0.1,0.1,0.15,-0.15,-0.1,
0,0.25+0.6,-0.15+0.6,-0.1+0.6,0.1,0.15,-0.15,-0.1,
0,0.25+0.6,-0.15+0.6,-0.1+0.6,0.1,0.15,-0.15,-0.1
),nrow=T,ncol=I,byr=T)
...
write(fit2ind, file="fit2ind.txt", ncol=length(fit2ind), append=TRUE, sep="\t")
...
write(fit3ind, file="fit3ind.txt", ncol=length(fit3ind), append=TRUE, sep="\t")

Converg <- c(Converg, inspect(fit3, what = "converged"))

estr <- parameterEstimates(fit3)$est[c(76:77, 73:75, 206:237, 1:32, c(33,37,41,45,49,53,57,61),78 )]
write(estr, file="estr.txt", ncol=length(estr), append=TRUE, sep="\t")


ser <- parameterEstimates(fit3)$se[c(76:77, 73:75, 206:237, 1:32, c(33,37,41,45,49,53,57,61),78 )]
write(ser, file="ser.txt", ncol=length(ser), append=TRUE, sep="\t")


pvaluer <- parameterEstimates(fit3)$pvalue[c(76:77, 73:75, 206:237, 1:32, c(33,37,41,45,49,53,57,61),78 )]
write(pvaluer, file="pvaluer.txt", ncol=length(pvaluer), append=TRUE, sep="\t")


cil <- parameterEstimates(fit3)$ci.lower[c(76:77, 73:75, 206:237, 1:32, c(33,37,41,45,49,53,57,61),78 )]
write(cil, file="cil.txt", ncol=length(cil), append=TRUE, sep="\t")

ciu <- parameterEstimates(fit3)$ci.upper[c(76:77, 73:75, 206:237, 1:32, c(33,37,41,45,49,53,57,61),78 )]
write(ciu, file="ciu.txt", ncol=length(ciu), append=TRUE, sep="\t")
...
write(pva, file="pva3.txt", ncol=length(pva), append=TRUE, sep="\t")
...
write(pval, file="pva31.txt", ncol=length(pval), append=TRUE, sep="\t")


write(Converg, file="Converg.txt", ncol=length(Converg), append=TRUE, sep="\t")

print(CIR)
et<-proc.time()
print((et-bt)[3])
write((et-bt)[3], file="timelav.txt", ncol=1, append=TRUE, sep="\t")
...
date()
save.image(paste("RAOla",".RData",sep=""))


#warnings()

Convergr<- rowSums( read.table("Converg.txt") )   

indcat <- Convergr==(23+21)


estres<-matrix(scan("estr.txt",nlines=CNUM),nrow=CNUM, byr=T)


truepa<-c(0, 0.2, 0.5, 0.1, 0.045, as.vector(A)/1.7, as.vector(L)/1.7, rep(0.5,I)/1.7,psx)
...
colMeans(cbind(abs(bias1[70:77]), sqrt(Rmse1[70:77]))) ##specifc slopes

cbind(bias1, sqrt(Rmse1))[c(1:5,78),] ##growth parameters

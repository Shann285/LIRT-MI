...
write(fit2ind, file="fit2indM.txt", ncol=length(fit2ind), append=TRUE, sep="\t")
...
write(fit3ind, file="fit3indM.txt", ncol=length(fit3ind), append=TRUE, sep="\t")

Converg <- c(Converg, inspect(fit3, what = "converged"))

estr <- parameterEstimates(fit3)$est[c(44:45, 41:43, 82:113, 1:8, 46 )]
write(estr, file="estrM.txt", ncol=length(estr), append=TRUE, sep="\t")


ser <- parameterEstimates(fit3)$se[c(44:45, 41:43, 82:113, 1:8, 46 )]
write(ser, file="serM.txt", ncol=length(ser), append=TRUE, sep="\t")


pvaluer <- parameterEstimates(fit3)$pvalue[c(44:45, 41:43, 82:113, 1:8, 46 )]
write(pvaluer, file="pvaluerM.txt", ncol=length(pvaluer), append=TRUE, sep="\t")


cil <- parameterEstimates(fit3)$ci.lower[c(44:45, 41:43, 82:113, 1:8, 46 )]
write(cil, file="cilM.txt", ncol=length(cil), append=TRUE, sep="\t")

ciu <- parameterEstimates(fit3)$ci.upper[c(44:45, 41:43, 82:113, 1:8, 46 )]
write(ciu, file="ciuM.txt", ncol=length(ciu), append=TRUE, sep="\t")


write(Converg, file="ConvergM.txt", ncol=length(Converg), append=TRUE, sep="\t")

print(CIR)
et<-proc.time()
print((et-bt)[3])
write((et-bt)[3], file="timelavM.txt", ncol=1, append=TRUE, sep="\t")

}

date()
save.image(paste("RAOlaM",".RData",sep=""))

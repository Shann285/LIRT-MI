...
for(CIR in 1:CNUM){
  ...
  for(n in 1:N){
    ...
    for(t in 1:T){
      for(i in 1:I){
        temp=exp(A[t,i]+the0[n,t]*L[t,i]+0.5*sf0[n,i])/(1+exp(A[t,i]+the0[n,t]*L[t,i]+0.5*sf0[n,i]))
...
model {
  for(n in 1:N){
    ...
    for(t in 1:T){
      Y[n,t] ~ bernoulli_logit(A0[t]+athre[n,t]*L0+Ls.*fac_dist_helper[n,(q+1):(q+I)]);   
    }
...
irt_1pl<-stan(model_code = model_code1, pars = c("mua0","Siga","A00","A0","L0","Ls"), data=data0, iter=8000, init=inits, chains=3, cores=3)

aa<-summary(irt_1pl, probs = c(0.025, 0.975), pars = c("mua0","Siga","A00","A0","L0","Ls"))


rm(list=ls()) #clear screen
setwd("C:/Users/dell/Desktop/U500L3M")


###############################
library(MASS)
library(rstan)
rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())

set.seed(123)
#data 
N=500       #sample size
q=2         #latent intercept/slope
T=4         #time points
I=8        #items
CNUM=50
YFILE="Y.txt"

###true parameter values
## correlation=0.201 
siga<-matrix(c(
0.5,0.045,
0.045,0.1),nrow=q,ncol=q)

K<-matrix(c(
1,0,
1,1,
1,2,
1,3),nrow=T,ncol=q,byr=T)

L<-matrix(c(
1,1.25,0.95,0.85,1.0,1.15,1.05,0.75,
1,1.25,0.95,0.85,1.0,1.15,1.05,0.75,
1,1.25,0.95,0.85,1.0,1.15,1.05,0.75,
1,1.25,0.95,0.85,1.0,1.15,1.05,0.75
),nrow=T,ncol=I,byr=T)

A<-matrix(c(
0,0.25,-0.15,-0.1,0.1,0.15,-0.15,-0.1,
0,0.25,-0.15,-0.1,0.1,0.15,-0.15,-0.1,
0,0.25+0.6,-0.15+0.6,-0.1+0.6,0.1,0.15,-0.15,-0.1,
0,0.25+0.6,-0.15+0.6,-0.1+0.6,0.1,0.15,-0.15,-0.1
),nrow=T,ncol=I,byr=T)

psx<-0.5

## compile the stan model
model_code1 = "
data {
  int<lower=1> N;
  int<lower=1> I;
  int<lower=2> T;
  int<lower=1> q;
  matrix[T,q] K;
  int<lower=0,upper=1> Y[N,T,I];
}

parameters {
  real mua0;
  vector<lower=0>[q] taua;
  cholesky_factor_corr[q] Cova;
  vector[T] psx[N];
  vector[I] A0[T];
  vector<lower=0>[I] L0;
  vector<lower=0>[I] Lamdai;
  vector[q] fac_dist_helper[N];
}

transformed parameters {
  real<lower=0> tau;
  vector[q] mua;
  vector[T] athre[N];
  vector[q] ath[N];
  vector[I] A00[T-1];
  matrix[q,q] Siga;
  mua[1] = 0;
  mua[2] = mua0;
  tau = 1-taua[1]*taua[1];
  Siga = diag_pre_multiply(taua,Cova) * diag_pre_multiply(taua,Cova)';
  for(n in 1:N){
    ath[n] = mua + diag_matrix(taua) * Cova * fac_dist_helper[n,1:q];
    athre[n] = K*ath[n] + psx[n];
  }
  for(t in 2:T){
    A00[t-1] = A0[t]-A0[1];
  }
}

model {
  for(n in 1:N){
    psx[n] ~ normal(0, sqrt(tau));
    for(t in 1:T){
      Y[n,t] ~ bernoulli_logit(A0[t]+athre[n,t]*L0 );   
    }
    fac_dist_helper[n] ~ normal(0,1);
  }
  A0[1] ~ normal(0,2);
  A0[2] ~ double_exponential(A0[1], 1 ./ sqrt(Lamdai) ); 
  A0[3] ~ double_exponential(A0[1], 1 ./ sqrt(Lamdai) ); 
  A0[4] ~ double_exponential(A0[1], 1 ./ sqrt(Lamdai) ); 
  L0 ~ normal(0,2);
  Lamdai ~ gamma(9, 3);
  mua0 ~ normal(0,2);
  taua ~ cauchy(0,2.5);
  Cova ~ lkj_corr_cholesky(2);
}
"

## 50 replications
for(CIR in 1:CNUM){
  bt<-proc.time()
  Y<-array(0, dim=c(N, T, I))
  Y<-array(scan(YFILE, skip=(CIR-1)*T*I, nlines=T*I), dim=c(N, T, I))

data0<-list(N=N, I=I, T=T, q=q, K=K, Y=Y)

inits = function() {
  init.values<-list(
    mua0 = runif((q-1),0,.1),
    taua = runif(q,0,0.1),
    Cova = matrix(c(1,0,0,0.9),nrow=q),
    L0 = runif(I,1.0,1.1),             
    A0 = matrix(runif(I*T,0,.1),nrow=T)
    )
  return(init.values);
}

irt_1pl<-stan(model_code = model_code1, pars = c("mua0","Siga","A00","A0","L0"), data=data0, iter=8000, init=inits, chains=3, cores=3)

aa<-summary(irt_1pl, probs = c(0.025, 0.975), pars = c("mua0","Siga","A00","A0","L0"))

write(get_num_divergent(irt_1pl), file="diverM.txt", ncol=1, append=TRUE, sep="\t")

write(aa$summary[,c(7)], file="RhhM.txt", ncol=length(aa$summary[,c(7)]), append=TRUE, sep="\t")
write(aa$summary[,c(1)], file="ameanM.txt", ncol=length(aa$summary[,c(1)]), append=TRUE, sep="\t")
write(aa$summary[,c(3)], file="asdM.txt", ncol=length(aa$summary[,c(3)]), append=TRUE, sep="\t")
write(aa$summary[,c(4)], file="ainlM.txt", ncol=length(aa$summary[,c(4)]), append=TRUE, sep="\t")
write(aa$summary[,c(5)], file="ainrM.txt", ncol=length(aa$summary[,c(5)]), append=TRUE, sep="\t")

print(CIR)
et<-proc.time()
print((et-bt)[3])
write((et-bt)[3], file="timeM.txt", ncol=1, append=TRUE, sep="\t")
}

date()
save.image(paste("RthresM",".RData",sep=""))



aR<-matrix(scan("RhhM.txt",nlines=CNUM),nrow=CNUM, byr=T)
ad<-matrix(scan("diverM.txt",nlines=CNUM),nrow=CNUM, byr=T)

aain<-which( (rowSums(aR[,c(1:3,5, 30:61, 62:69)]>1.05)==0) & (ad==0) )

length(aain)/CNUM  ##convergence rate


amean<-matrix(scan("amean.txt",nlines=CNUM),nrow=CNUM, byr=T)

truepa<-c(0.2, 0.5,0.045,0.1, as.vector(t(A)),as.vector(L[1,]))


bias1 <- apply(amean[aain, c(1:3,5, 30:61, 62:69)], MARGIN=2, FUN=mean)-truepa

Rmse1 <- colMeans((amean[aain, c(1:3,5, 30:61, 62:69)] - matrix(rep(truepa, each=length(aain)), nrow=length(aain)))^2)


colMeans(abs(cbind(bias1, sqrt(Rmse1))[(4*I+5):(5*I+4),])) ##a1

colMeans(abs(cbind(bias1, sqrt(Rmse1))[5:(I+4),])) ##d1

colMeans(abs(cbind(bias1, sqrt(Rmse1))[c((I+5):(2*I+4), (2*I+5),(2*I+6),(2*I+8):(3*I+4), (3*I+5),(3*I+6),(3*I+8):(4*I+4) ),])) ##d2-d4 non-DIF increments

colMeans(abs(cbind(bias1, sqrt(Rmse1))[c((2*I+7),(3*I+7)),])) ##d2-d4 DIF increments


cbind(bias1, sqrt(Rmse1))[1:4,] ##growth parameters

